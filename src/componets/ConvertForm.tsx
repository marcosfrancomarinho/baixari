...
export function ConvertForm() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/documents/convert/pdf`, {
        method: 'POST',
        body: form,
      ...
      if (!response.ok) {
        ...
        throw new Error(data?.error || `Erro na conversão (HTTP ${response.status}).`);
      }
      const blob = await response.blob();
      if (!blob.size) throw new Error('O servidor retornou um PDF vazio.');
      savePdf(blob, 'documentos.pdf');
      setAlert({ message: 'PDF gerado. Download iniciado.', type: 'success' });
    } catch (error) {
      setAlert({
  ...
  return (
    <section className='w-full flex items-center justify-center px-4 py-10 bg-white text-black'>
      <div className='w-full max-w-xl border border-gray-300 p-6 flex flex-col gap-4 bg-white'>
        ...
        <p className='text-sm text-gray-600'>Adicione PDFs e imagens. A ordem abaixo será mantida no PDF final.</p>
        ...
        {loading && <p role='status' className='text-sm text-gray-600'>Enviando e processando os documentos. Arquivos maiores podem demorar.</p>}
        ...
        {loading && <button type='button' onClick={() => controller.current?.abort()} className='border border-gray-400 py-2 font-medium'>Cancelar conversão</button>}
      </div>
    </section>
  );
}
